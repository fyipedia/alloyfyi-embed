/* alloyfyi-embed v1.0.0 | MIT | https://widget.alloyfyi.com */
...
function parseComposition(comp) {
  return Object.entries(comp).map(([el, range]) => {
    const nums = range.match(/[\d.]+/g);
    const avg = nums ? nums.reduce((a, b) => a + parseFloat(b), 0) / nums.length : 0;
    return { element: el, pct: avg, range };
...
// src/tools/alloy-radar.ts
function esc2(s) {
  return s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
function renderChart(container, values, name, slug, config) {
  const entityUrl = `https://${config.domain}/alloys/${esc2(slug)}/`;
  const svg = radarSvg({ values, labels: ALLOY_AXES, accent: config.accent, size: 200, maxValue: 10 });
  const valueRows = ALLOY_AXES.map(
    (axis, i) => {
      var _a, _b;
      return `<div style="display:flex;justify-content:space-between;padding:2px 0;font-size:12px;"><span style="color:#64748b;">${esc2(axis)}</span><span style="font-weight:600;">${(_b = (_a = values[i]) == null ? void 0 : _a.toFixed(1)) != null ? _b : "\u2014"}/10</span></div>`;
    }
  ).join("");
  container.innerHTML = `
    <div class="sciencefyi-radar-tool" style="text-align:center;">
      ${name ? `<div style="font-size:0.95rem;font-weight:600;margin-bottom:8px;">${esc2(name)}</div>` : ""}
      <div style="display:inline-block;">${svg}</div>
      <div style="margin-top:8px;text-align:left;">${valueRows}</div>
      ${slug ? `<div style="margin-top:8px;text-align:center;"><a href="${entityUrl}" target="_blank" rel="noopener" style="font-size:12px;color:${config.accent};text-decoration:none;">View on ${esc2(config.name)} \u2197</a></div>` : ""}
    </div>
    ${poweredByHTML(config)}
  `;
}
function initAlloyRadar(el, config) {
  var _a, _b, _c, _d, _e, _f, _g, _h;
  const shadow = createShadow(el, config);
  const container = createWidgetRoot(shadow, el, "sciencefyi-alloy-radar-widget");
  const slug = (_a = el.dataset.slug) != null ? _a : "";
  if (slug) {
    renderLoading(container);
    fetchAPI(config.apiBase, `alloys/${slug}/`).then((data) => {
      var _a2, _b2, _c2, _d2, _e2, _f2, _g2, _h2, _i;
      const values = [
        Number((_b2 = (_a2 = data.strength_rating) != null ? _a2 : data.tensile_rating) != null ? _b2 : 5),
        Number((_c2 = data.corrosion_rating) != null ? _c2 : 5),
        Number((_d2 = data.lightweight_rating) != null ? _d2 : 5),
        Number((_e2 = data.machinability_rating) != null ? _e2 : 5),
        Number((_f2 = data.cost_rating) != null ? _f2 : 5),
        Number((_h2 = (_g2 = data.temperature_rating) != null ? _g2 : data.heat_rating) != null ? _h2 : 5)
      ];
      renderChart(container, values, String((_i = data.name) != null ? _i : ""), slug, config);
    }).catch(() => renderError(container, `Alloy "${esc2(slug)}" not found.`, config));
  } else {
    const values = [
      Number((_b = el.dataset.strength) != null ? _b : 5),
      Number((_c = el.dataset.corrosion) != null ? _c : 5),
      Number((_d = el.dataset.lightweight) != null ? _d : 5),
      Number((_e = el.dataset.machinability) != null ? _e : 5),
      Number((_f = el.dataset.cost) != null ? _f : 5),
      Number((_g = el.dataset.temperature) != null ? _g : 5)
    ];
    renderChart(container, values, (_h = el.dataset.name) != null ? _h : "", "", config);
  }
}

// src/tools/composition-bar.ts
function esc3(s) {
  return s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
var ELEMENT_COLORS = [
...
function renderBar(container, data, slug, config) {
  var _a;
  const name = String((_a = data.name) != null ? _a : slug);
  const entityUrl = `https://${config.domain}/alloys/${esc3(slug)}/`;
  const comp = data.composition;
  if (!comp || Object.keys(comp).length === 0) {
    container.innerHTML = `
      <div style="padding:8px;text-align:center;color:#64748b;font-size:13px;">No composition data available for this alloy.</div>
      <div style="margin-top:8px;text-align:center;"><a href="${entityUrl}" target="_blank" rel="noopener" style="font-size:12px;color:${config.accent};">View on ${esc3(config.name)} \u2197</a></div>
      ${poweredByHTML(config)}
    `;
    return;
  }
  const parsed = parseComposition(comp);
  const total = parsed.reduce((s, d) => s + d.pct, 0);
  const normalized = total > 0 ? parsed.map((d) => ({ ...d, pct: d.pct / total * 100 })) : parsed;
  const barSegments = normalized.map((d, i) => {
    const color = ELEMENT_COLORS[i % ELEMENT_COLORS.length];
    const width = d.pct.toFixed(2);
    const label = d.pct >= 5 ? `<span style="position:absolute;left:50%;transform:translateX(-50%);font-size:9px;color:#fff;font-weight:600;pointer-events:none;">${d.pct >= 10 ? d.pct.toFixed(0) + "%" : ""}</span>` : "";
    return `<div title="${esc3(d.element)}: ${esc3(d.range)}" style="position:relative;width:${width}%;background:${color};height:100%;display:inline-block;">${label}</div>`;
  }).join("");
  const legendItems = normalized.map((d, i) => {
    const color = ELEMENT_COLORS[i % ELEMENT_COLORS.length];
    return `<div style="display:flex;align-items:center;gap:4px;font-size:11px;">
      ...
      <span style="font-weight:600;">${esc3(d.element)}</span>
      <span style="color:#94a3b8;">${esc3(d.range)}%</span>
    </div>`;
  }).join("");
  container.innerHTML = `
    <div class="sciencefyi-comp-bar-tool">
      <div style="font-size:0.9rem;font-weight:600;margin-bottom:8px;">${esc3(name)} \u2014 Composition</div>
      <div style="height:28px;border-radius:6px;overflow:hidden;display:flex;margin-bottom:10px;">${barSegments}</div>
      <div style="display:flex;flex-wrap:wrap;gap:6px 12px;">${legendItems}</div>
      <div style="margin-top:10px;">
        <a href="${entityUrl}" target="_blank" rel="noopener" style="font-size:12px;color:${config.accent};text-decoration:none;">View on ${esc3(config.name)} \u2197</a>
      </div>
    </div>
    ${poweredByHTML(config)}
  `;
}
function initCompositionBar(el, config) {
  var _a;
  const shadow = createShadow(el, config);
  const container = createWidgetRoot(shadow, el, "sciencefyi-composition-bar-widget");
  const slug = (_a = el.dataset.slug) != null ? _a : "";
  if (!slug) {
  ...
  renderLoading(container);
  fetchAPI(config.apiBase, `alloys/${slug}/`).then((data) => renderBar(container, data, slug, config)).catch(() => renderError(container, `Alloy "${esc3(slug)}" not found.`, config));
}

// src/_entry_alloyfyi.ts
function initWidget(el, type, config) {
  ...
  switch (type) {
    ...
    case "search":
      ...
      break;
    case "alloy-radar":
      initAlloyRadar(el, config);
      break;
    case "composition-bar":
      initCompositionBar(el, config);
      break;
    default:
      break;
  }
...
(function bootstrap() {
  const config = '{"site":"alloyfyi","name":"AlloyFYI","domain":"alloyfyi.com","accent":"#2563EB","attribute":"data-alloyfyi","apiBase":"https://alloyfyi.com/api/v1/","searchPath":"/search/","entityName":"Alloys","entitySlug":"alloys"}';
...
function makeWidgetElement(widgetType, initFn, domainAttrs) {
  ...
  return class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._syncDataAttrs();
      initFn(this, '{"site":"alloyfyi","name":"AlloyFYI","domain":"alloyfyi.com","accent":"#2563EB","attribute":"data-alloyfyi","apiBase":"https://alloyfyi.com/api/v1/","searchPath":"/search/","entityName":"Alloys","entitySlug":"alloys"}');
    }
    ...
    _syncDataAttrs() {
      const attrKey = '{"site":"alloyfyi","name":"AlloyFYI","domain":"alloyfyi.com","accent":"#2563EB","attribute":"data-alloyfyi","apiBase":"https://alloyfyi.com/api/v1/","searchPath":"/search/","entityName":"Alloys","entitySlug":"alloys"}'.attribute.replace("data-", "");
...
(function registerElements() {
  ...
  const site = '{"site":"alloyfyi","name":"AlloyFYI","domain":"alloyfyi.com","accent":"#2563EB","attribute":"data-alloyfyi","apiBase":"https://alloyfyi.com/api/v1/","searchPath":"/search/","entityName":"Alloys","entitySlug":"alloys"}'.site;
  const defs = [
    ...
    [`${site}-alloy-radar`, initAlloyRadar, ["slug"]],
    [`${site}-composition-bar`, initCompositionBar, ["slug"]]
  ];
